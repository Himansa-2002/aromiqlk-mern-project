...
const ProductCard = ({ product }) => {
  ...
  const brandSlug = typeof product.brand === "object" ? product.brand?.slug : null;

  return (
    <div className="group border border-line bg-panel hover:border-gold transition-colors duration-300">
      ...
      <div className="p-4 pb-5">
        {brandSlug ? (
          <Link to={`/brand/${brandSlug}`} className="text-[11px] uppercase tracking-wider text-gold hover:text-gold-bright">
            {brandName}
          </Link>
        ) : (
        ...
        <Link to={`/product/${slug}`}>
          <h4 className="font-display text-lg mt-1.5 mb-2.5 text-warm hover:text-gold-bright transition-colors">{name}</h4>
        </Link>
