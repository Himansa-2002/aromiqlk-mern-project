...
export default function AdminCategories() {
  ...
  const load = () => {
    setLoading(true);
    fetch("/api/categories").then((r) => r.json()).then(setItems).finally(() => setLoading(false));
  };
  useEffect(load, []);

  const handleSubmit = async (e) => {
    e.preventDefault();
    const url = editingId ? `/api/categories/admin/${editingId}` : "/api/categories/admin";
  ...
  const startEdit = (c) => { setForm({ name: c.name, slug: c.slug, description: c.description || "" }); setEditingId(c._id); };
  const handleDelete = async (id) => { if (!confirm("Delete this category?")) return; await fetch(`/api/categories/admin/${id}`, { method: "DELETE" }); load(); };
  ...
  return (
    <div className="p-8">
      ...
      <p className="text-muted text-sm mb-8">{items.length} categor{items.length === 1 ? "y" : "ies"}</p>
      ...
      <form onSubmit={handleSubmit} className="border border-line bg-panel p-6 mb-8 grid grid-cols-3 gap-4 items-end">
        ...
        <button type="submit" className="px-6 py-2.5 text-xs uppercase tracking-widest bg-gold text-ink font-medium h-[42px]">{editingId ? "Update" : "+ Add Category"}</button>
      </form>
        ...
        <div className="border border-line bg-panel">
          {items.map((c) => (
            <div key={c._id} className="flex items-center justify-between p-4 border-b border-line last:border-b-0">
              <div><span className="text-sm">{c.name}</span> <span className="text-muted text-xs ml-2">/{c.slug}</span></div>
              <div>
          ...
          {items.length === 0 && <p className="text-muted text-sm text-center py-10">No categories yet.</p>}
        </div>
      )}
    </div>
  );
